...
VERSION=
commit=false
url=https://github.com/zodiacproject/zodiac
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the zodiac, gitian-builder, gitian.sigs, and zodiac-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/zodiacproject/zodiac
...
then
	# Make output folder
	mkdir -p "./zodiac-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit zodiac=${COMMIT} --url zodiac=${url} ../zodiac/contrib/gitian-descriptors/gitian-linux.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-linux --destination ../gitian.sigs/ ../zodiac/contrib/gitian-descriptors/gitian-linux.yml
	    mv build/out/zodiac-*.tar.gz build/out/src/zodiac-*.tar.gz ../zodiac-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit zodiac=${COMMIT} --url zodiac=${url} ../zodiac/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/zodiac-*.zip build/out/zodiac-*.exe ../zodiac-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit zodiac=${COMMIT} --url zodiac=${url} ../zodiac/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/zodiac-*.tar.gz build/out/zodiac-*.dmg ../zodiac-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit zodiac=${COMMIT} --url zodiac=${url} ../zodiac/contrib/gitian-descriptors/gitian-aarch64.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-aarch64 --destination ../gitian.sigs/ ../zodiac/contrib/gitian-descriptors/gitian-aarch64.yml
	    mv build/out/zodiac-*.tar.gz build/out/src/zodiac-*.tar.gz ../zodiac-binaries/${VERSION}
	fi
	popd || exit
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../zodiac/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/zodiac-*win64-setup.exe ../zodiac-binaries/${VERSION}
	    mv build/out/zodiac-*win32-setup.exe ../zodiac-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../zodiac/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/zodiac-osx-signed.dmg ../zodiac-binaries/${VERSION}/zodiac-${VERSION}-osx.dmg
	fi
	popd || exit
