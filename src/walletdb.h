...
public:
    ...
    bool WriteZZDCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZZDCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZZDCSeed_deprecated(uint256& seed);
    bool EraseZZDCSeed();
    bool EraseZZDCSeed_deprecated();

    bool WriteZZDCCount(const uint32_t& nCount);
    bool ReadZZDCCount(uint32_t& nCount);
