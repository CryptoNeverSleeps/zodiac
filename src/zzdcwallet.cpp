...
CzZDCWallet::CzZDCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zzdc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZZDCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZZDCSeed_deprecated()) {
                    LogPrintf("%s: Updated zZDC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zzdc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zzdc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzZDCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZZDCCount(nCountLastUsed);
    else if (!walletdb.ReadZZDCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzZDCWallet::Lock()
{
    seedMaster = 0;
}

void CzZDCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzZDCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZZDC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzZDCWallet::LoadMintPoolFromDB()
{
...
void CzZDCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzZDCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzZDCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zzdcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzZDCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZZDC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zzdcTracker which also adds to database
    pwalletMain->zzdcTracker->Add(dMint, true);
...
void CzZDCWallet::SeedToZZDC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzZDCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzZDCWallet::GenerateDeterministicZZDC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzZDCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzZDCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
