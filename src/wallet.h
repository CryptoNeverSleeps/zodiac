...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ZDC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zZDC send
enum ZerocoinSpendStatus {
    ZZDC_SPEND_OKAY = 0,                            // No error
    ZZDC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZZDC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZZDC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZZDC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZZDC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZZDC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZZDC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZZDC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZZDC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZZDC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZZDC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZZDC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZZDC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZZDC_SPENT_USED_ZZDC = 14,                      // Coin has already been spend
    ZZDC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZZDC_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZZdcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZZDCOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzzdcAuto) const;
    ...
    */
    ...
    CzZDCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzZDCTracker> zzdcTracker;
    ...
    void setZWallet(CzZDCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zzdcTracker = std::unique_ptr<CzZDCTracker>(new CzZDCTracker(strWalletFile));
    }

    CzZDCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZZdcAutoBackups(bool fEnabled)
    {
