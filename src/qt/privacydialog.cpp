                                                          ...
                                                          currentBalance(-1)
{
    ...
    ui->zZDCpayAmount->setAttribute(Qt::WA_MacShowFocusRect, 0);
    

    // "Spending 999999 zZDC ought to be enough for anybody." - Bill Gates, 2017
    ui->zZDCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZDCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZDCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZDC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZDC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZDC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZDC in ") +
...
void PrivacyDialog::on_pushButtonSpendzZDC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_23_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZDC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZDC
        sendzZDC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZDC
    sendzZDC();
}

void PrivacyDialog::on_pushButtonZZdcControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZZdcControlDialog* zZdcControl = new ZZdcControlDialog(this);
    zZdcControl->setModel(walletModel);
    zZdcControl->exec();
}

void PrivacyDialog::setZZdcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZDCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZDC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Zodiac Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZDCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZDC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZDC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZDC</b>";
    ...
    // use mints from zZDC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZZdcControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZZdcControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zZDC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zZDC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZZDC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zZDC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zZDC"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zZDC was spent successfully update the addressbook with the label
    ...
    // Clear zzdc selector in case it was used
    ZZdcControlDialog::setSelectedMints.clear();
    ui->labelzZDCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZDC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Zdc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZDC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zzdcTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZDC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZDC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZDC "));
    ui->labelzZDCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZDCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK23Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzZDC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zZDC
        ui->pushButtonMintzZDC->setEnabled(false);
        ui->pushButtonMintzZDC->setToolTip(tr("zZDC is currently disabled due to maintenance."));

        // Spend zZDC
        ui->pushButtonSpendzZDC->setEnabled(false);
        ui->pushButtonSpendzZDC->setToolTip(tr("zZDC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zZDC
        ui->pushButtonMintzZDC->setEnabled(true);
        ui->pushButtonMintzZDC->setToolTip(tr("PrivacyDialog", "Enter an amount of ZDC to convert to zZDC", 0));

        // Spend zZDC
        ui->pushButtonSpendzZDC->setEnabled(true);
        ui->pushButtonSpendzZDC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}