...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ZDC);
    ...
    if (!settings.contains("nAnonymizeZodiacAmount"))
        settings.setValue("nAnonymizeZodiacAmount", 1000);

    nAnonymizeZodiacAmount = settings.value("nAnonymizeZodiacAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeZodiacAmount"))
        SoftSetArg("-anonymizezodiacamount", settings.value("nAnonymizeZodiacAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in zodiac.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZodiacAmount:
            return QVariant(nAnonymizeZodiacAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeZodiacAmount:
            nAnonymizeZodiacAmount = value.toInt();
            settings.setValue("nAnonymizeZodiacAmount", nAnonymizeZodiacAmount);
            emit anonymizeZodiacAmountChanged(nAnonymizeZodiacAmount);
            break;
        case CoinControlFeatures:
