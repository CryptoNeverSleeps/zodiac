...
#include "ui_zzdccontroldialog.h"
...
std::set<std::string> ZZdcControlDialog::setSelectedMints;
std::set<CMintMeta> ZZdcControlDialog::setMints;

ZZdcControlDialog::ZZdcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZdcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZZdcControlDialog::~ZZdcControlDialog()
{
    delete ui;
}

void ZZdcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZdcControlDialog::updateList()
{
...
void ZZdcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZdcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZZdc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZdcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZZdcControlDialog::GetSelectedMints()
{
...
void ZZdcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
